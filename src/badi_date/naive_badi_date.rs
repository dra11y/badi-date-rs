...
impl NaiveBadiDate {
    /// Create a new [`NaiveBadiDate`] given day, [`BadiMonth`], and year; checks for validity
...
impl BadiDateLike for NaiveBadiDate {
    fn year(&self) -> u8 {
        self.year
    }
    ...
    fn with_day(&self, day: u16) -> Result<NaiveBadiDate, BadiDateError> {
        Self::new(self.year, self.month, day)
    }

    fn with_month(&self, month: BadiMonth) -> Result<NaiveBadiDate, BadiDateError> {
    ...
    fn with_ymd(
        ...
        month: BadiMonth,
        day: u16,
    ) -> Result<NaiveBadiDate, BadiDateError> {
